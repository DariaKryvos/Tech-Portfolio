    ...
    <div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-24 pb-10 sm:pt-28 sm:pb-12 md:pt-40 md:pb-20">
        ...
        <p className="text-gray-400 max-w-lg text-sm md:text-base leading-relaxed mb-4 sm:mb-6">
          Software developer who builds websites, apps, and online platforms. Also runs a digital agency that does everything from design to security.
        </p>
        <div className="flex flex-wrap gap-3 sm:gap-4 text-xs sm:text-sm text-gray-500 mb-6 sm:mb-8">
          <span className="flex items-center gap-1.5"><MapPin size={12} /> The Netherlands</span>
          <span className="flex items-center gap-1.5"><Mail size={12} /> megokruto@gmail.com</span>
          <span className="flex items-center gap-1.5"><Phone size={12} /> +31 6 27 58 69 65</span>
        </div>
      ...
      <section className="border-t border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16 md:py-24">
          <div className="flex items-center gap-2.5 sm:gap-3 mb-8 sm:mb-10">
            <Briefcase size={18} className="text-orange-400" strokeWidth={1.5} />
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white">Experience</h2>
          </div>

          <div className="space-y-4 sm:space-y-6">
            {experience.map((job) => (
              <div key={`${job.company}-${job.role}`} className="glass-card rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1.5 sm:gap-2 mb-3 sm:mb-4">
                  <div>
                    <h3 className="text-base sm:text-lg md:text-xl font-semibold text-white">{job.role}</h3>
                    <p className="text-xs sm:text-sm text-orange-400/70">{job.company}</p>
                  </div>
                  <span className="text-xs sm:text-sm text-gray-500 whitespace-nowrap">{job.period}</span>
                </div>
                <p className="text-xs sm:text-sm text-gray-400 leading-relaxed mb-3 sm:mb-4">{job.description}</p>
                <ul className="space-y-1.5 sm:space-y-2">
                  {job.highlights.map((h) => (
                    <li key={h} className="flex items-start gap-2 text-xs sm:text-sm text-gray-400">
                      <ArrowRight size={10} className="text-orange-400/60 mt-1 shrink-0 sm:w-3 sm:h-3" />
                      {h}
                    </li>
                  ))}
                </ul>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16 md:py-24">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
            {Object.entries(skills).map(([category, items]) => (
              <div key={category} className="glass-card rounded-xl sm:rounded-2xl p-4 sm:p-6">
                <h3 className="text-[10px] sm:text-sm uppercase tracking-widest text-gray-500 font-medium mb-3 sm:mb-4">{category}</h3>
          ...
          <div className="grid md:grid-cols-2 gap-8 sm:gap-10 md:gap-16">
            ...
            <div>
              <div className="flex items-center gap-2.5 sm:gap-3 mb-6 sm:mb-8">
                <GraduationCap size={18} className="text-orange-400" strokeWidth={1.5} />
                <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white">Education</h2>
              </div>
              {education.map((edu) => (
                <div key={edu.degree} className="glass-card rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1.5 sm:gap-2 mb-2 sm:mb-3">
                    <h3 className="text-sm sm:text-base md:text-lg font-semibold text-white">{edu.degree}</h3>
                    <span className="text-xs sm:text-sm text-gray-500 whitespace-nowrap">{edu.period}</span>
                  </div>
                  <p className="text-xs sm:text-sm text-orange-400/70 mb-1.5 sm:mb-2">{edu.school}</p>
                  <p className="text-xs sm:text-sm text-gray-400 leading-relaxed">{edu.details}</p>
                </div>
              ))}
            </div>
            ...
            <div>
              <div className="flex items-center gap-2.5 sm:gap-3 mb-6 sm:mb-8">
                <Shield size={18} className="text-orange-400" strokeWidth={1.5} />
                <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white">Certifications</h2>
              </div>
              <div className="space-y-2.5 sm:space-y-3">
                {certifications.map((cert) => (
                  <div key={cert} className="glass-card rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 flex items-center gap-2.5 sm:gap-3">
                    <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-orange-400/10 flex items-center justify-center shrink-0">
                      <Shield size={12} className="text-orange-400" />
                    </div>
                    <span className="text-xs sm:text-sm md:text-base text-white font-medium">{cert}</span>
                  </div>
                ))}
              </div>
            </div>
